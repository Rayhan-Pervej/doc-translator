...
2. **Chunk** - split into groups sized by output token budget (max 6000 tokens, max 80 strings per chunk)
3. **Translate** - send each chunk to Claude and get back a JSON array of English strings
4. **Validate** - verify the count matches exactly; retry up to 3 times if not
5. **Fall back** - if a chunk still has gaps after retries, translate missing strings one by one
...
│
└── translator/
    ├── client.py            <- Bedrock boto3 client + token tracking
    ├── common.py            <- batch_translate(), chunking, JSON array parsing, helpers
